...
// starts (lib/actions.ts), unlike packages/payments' default handler,
// which creates the row lazily on the webhook. So this route composes the
// shared signature-verification primitive with an update rather than using
// the package's default create-based POST -- idempotent because a second
// delivery finds no row still PENDING to update.
